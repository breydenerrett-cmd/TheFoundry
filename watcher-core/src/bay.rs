...
mod tests {
    ...
    fn known_repos_map_to_their_bay() {
        assert_eq!(
            resolve_bay(Some("/home/user/aisportsanalysis")),
            "SPORTS LAB"
        );
        assert_eq!(
            resolve_bay(Some("github.com/breydenerrett-cmd/resume-business")),
            "AI BUSINESS COMPLEX"
        );
    }
    ...
    fn unmatched_but_present_hint_is_experiments_not_silently_dropped() {
        assert_eq!(
            resolve_bay(Some("/home/user/some-new-thing")),
            "EXPERIMENTS"
        );
    }
}
